...
parser = argparse.ArgumentParser(
    description="cifar10 classification models, distributed data parallel test on single GPU"
)
parser.add_argument("--lr", default=0.1, help="")
parser.add_argument("--batch_size", type=int, default=2048, help="")
parser.add_argument("--num_workers", type=int, default=0, help="")
parser.add_argument(
    "--world_size", type=int, default=68, help="number of processes to simulate DDP"
)
parser.add_argument(
    "--master_port", type=str, default="12355", help="master port for communication"
)
...
def run_training(rank, world_size, args):
    ...
    for _ in range(100):
        for batch_idx, (inputs, targets) in enumerate(train_loader):
            ...
            if batch_idx % 10 == 0:
                print(
                    f"Rank {rank}, Batch {batch_idx}, Loss: {loss.item():.4f}, Images/sec: {images_per_sec:.2f}"
                )
    ...
    print(
        f"Rank {rank}: Training completed in {total_time:.2f}s, Avg performance: {avg_perf:.2f} images/sec"
    )
...
if __name__ == "__main__":
    ...
    mp.spawn(f, nprocs=10, join=True)
