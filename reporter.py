...
class Reporter:
    ...
    def get_experiment_paths(
        ...
        paths = sorted(
            paths,
            key=lambda p: tuple(
                int(x) for x in re.search(r"top-(\d+)_kappa_(\d+)", p).groups()
            ),
        )
    ...
    def plot_single_experiment_set(
        ...
        if len(self.metrics_paths) == 0:
            logger.info(
                f"No data found for ({vision_model.value}, {vision_dataset.value}), skipping."
            )
            return
...
class LaTeXTableGenerator:
    ...
    def generate_tables(self, topK: int = 8000, kappa: int = 7000, genereate_csv=False):
        ...
        save_latex(
            ...
            file_name="forget_loss",
            caption=f"Unlearning performance as forget set loss when (TopK, $\kappa$) is ({topK}, {kappa}).",
        )
        save_latex(
            df=forget_set_score_df,
            file_name="forget_score",
            caption=f"Unlearning performance as forget set accuracy when (TopK, $\kappa$) is ({topK}, {kappa}).",
        )
        save_latex(
            df=retain_set_loss_df,
            file_name="retain_loss",
            caption=f"Model degradation measured as retain set loss when (TopK, $\kappa$) is ({topK}, {kappa}).",
        )
        save_latex(
            df=retain_set_score_df,
            file_name="retain_score",
            caption=f"Model degradation measured as retain set accuracy when (TopK, $\kappa$) is ({topK}, {kappa}).",
        )

        # summarize
        logger.info(
            f"Generated {num_success} / {len(list(product(supported_datasets, model_architectures)))} plots."
        )
